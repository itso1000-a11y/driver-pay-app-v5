# v5.3.2 — PHONE REGRESSION / REST / UPDATE CORRECTION

**Status:** SOURCE-QA; narrow correction for partial time entry, factual rest continuity through non-work days, historic driver presentation, and controlled PWA update activation. Final executed evidence is in `PHONE_REGRESSION_FIX_QA_REPORT_v5.3.2.md`. Physical phone acceptance remains pending.

## Consolidated through v5.3.2

- Incomplete editable time values remain UI state and are excluded from factual migration until they are complete `HH:MM` values.
- A factual rest interval remains continuous across `Off` and `Holiday` days; it ends only at the next factual work start.
- Weekly-rest and compensation presentation uses the established driver-facing EN/BG wording and historical status palettes without internal engine terminology.
- An installed service worker remains waiting until the driver explicitly selects Update; that action sends `SKIP_WAITING` and the existing `controllerchange` handler reloads once.
- Phase 1–7 legal modules and Pay Engine calculations remain unchanged.

---
# v5.3.0 — REST ENGINE v1 PHASE 8 UI ACTIVATION

**Status:** SOURCE-QA; production EngineEvaluation adapter and UI integration implemented. Automated acceptance and independent review evidence are recorded in `PHASE8_QA_REPORT_v5.3.0.md`. Physical phone acceptance remains a user step.

## Consolidated through v5.3.0

- Phase 1–7 legal modules remain unchanged from the accepted v5.2.60 baseline.
- Factual Rest Card, separate compensation/warning panel, rolling weekly-rest planning and Week Preview now consume the authoritative EngineEvaluation and migration review state.
- Historical navigation and End Week remain factual-input and pay-workflow operations; legacy weekly-rest/compensation ledgers no longer set the production legal result.
- EN/BG presentation and mobile/desktop integration are covered by the Phase 8 QA report.

---
# v5.2.60 — REST ENGINE v1 PHASE 6+7 MIGRATION RECOVERY / PRECEDENCE CORRECTION

**Status:** SOURCE-QA; the narrow interrupted re-migration recovery and legacy factual-source precedence correction is implemented. Phase 8 remains not started.

## v5.2.60 correction scope

- Promotes a valid staged migration over an older current migration after interruption between staging and commit.
- Updates `current` and the completion marker to the staged snapshot, then removes resolved staging while preserving every legacy key.
- Corrects factual precedence to archive baseline → saved week → live/current.
- Preserves conflicting top-priority `days` and `driverApp_days` values as REVIEW evidence without choosing an alias or falling back to stale saved/archive data.
- Adds T243–T245 and retains T192–T242.
- Changes no Phase 1–5 functional source, evaluator, warning logic, UI, runtime entry point or dependency.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.60

The correction evidence is recorded in `PHASE6_7_MIGRATION_RECOVERY_PRECEDENCE_QA_REPORT_v5.2.60.md`.

---

# v5.2.59 — REST ENGINE v1 COMBINED PHASE 6+7

**Status:** SOURCE-QA; factual historical recomputation/migration and derived warning aggregation are implemented in shadow modules. Phase 8 runtime/UI activation is not started.

## v5.2.59 scope

- Adds the authoritative `evaluateRestEngine` pipeline from factual inputs, explicit `asOf`, explicit ruleset/policy configuration and accepted Phase 1–5 modules.
- Adds complete authoritative-input hashing, stable evaluation fingerprints, immutable results and write-through-only disposable derived cache support.
- Recomputes the full downstream graph after factual corrections; unsupported old obligations, blocks and outcomes disappear naturally.
- Adds versioned, namespaced, non-destructive legacy fact migration with snapshot, staging, idempotent replay, interrupted-write recovery and rollback/audit access.
- Excludes legacy weekly-rest and compensation ledgers from factual authority and preserves unrelated application data unchanged.
- Adds branch-aware warning aggregation with separate GREEN, YELLOW and RED semantics, Case A/B/C preservation and the rule that one valid branch prevents RED.
- Keeps legal deadlines, earliest candidate legal deadlines and planning latest-start targets separate.
- Adds T192–T242 and retains T01–T191 plus Phase 1 A–H.
- Does not connect the new pipeline or warnings to visible runtime UI. `App.tsx`, `main.tsx`, CSS, labels, layout and workflow remain unchanged.

## Consolidated through v5.2.59

The implementation and validation evidence is recorded in `PHASE6_7_RECOMPUTATION_WARNING_AGGREGATION_QA_REPORT_v5.2.59.md`.

---

# v5.2.58 — REST ENGINE v1 PHASE 5 TRANSITIVE UNCERTAINTY PROVENANCE CORRECTION

**Status:** SOURCE-QA; the narrow transitive uncertainty compatibility provenance correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.58 correction scope

- Carries canonical original factual REVIEW requirements through every `ALLOCATION_DEPENDENCY` instead of retaining only the immediate clear-record identity.
- Merges inherited requirement states by original rest, obligation and evidence, retaining the strictest required-by cutoff and rejecting incompatible ordinary-base reuse transitively.
- Preserves compatible requirements from independent factual rests and from one shareable regular Weekly Rest base with sufficient non-overlapping capacity.
- Keeps provenance for obligations that later receive a confirmed primary block so that the possible absence of that block remains available to downstream compatibility analysis without downgrading the positive result.
- Deduplicates equivalent requirement states deterministically and keeps fixed-point propagation bounded by obligation count.
- Preserves primary allocation, priority, full-fit/skip-fit, sequential capacity consumption and all accepted Phase 5 timing and uncertainty rules.
- Adds T188–T191 and retains T100–T187.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.58

The correction evidence is recorded in `PHASE5_TRANSITIVE_UNCERTAINTY_PROVENANCE_QA_REPORT_v5.2.58.md`.

---

# v5.2.57 — REST ENGINE v1 PHASE 5 JOINT UNCERTAINTY COMPATIBILITY CORRECTION

**Status:** SOURCE-QA; the narrow jointly compatible higher-obligation uncertainty correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.57 correction scope

- Extends bounded alternate dependency analysis from one uncertain higher block to finite jointly compatible release sets on one contested clear allocation record.
- Combines independent earlier repayment evidence while rejecting competing uses of one non-shareable ordinary rest/base/capacity.
- Allows shared evidence only when one common chronology and an accepted shareable regular Weekly Rest base can fit all selected higher obligations sequentially.
- Preserves all other actual blocks and reserved ranges, then reuses the accepted deterministic full-fit/skip-fit alternate allocator over one copied capacity state.
- Records joint dependency provenance and adds REVIEW only; primary allocations remain unchanged and no favourable history or alternate block becomes factual.
- Bounds the search to actual blocks and their finite structured evidence on one record, pruning incompatible states and deduplicating release sets.
- Adds T184–T187 and retains T100–T183.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.57

The correction evidence is recorded in `PHASE5_JOINT_UNCERTAINTY_COMPATIBILITY_QA_REPORT_v5.2.57.md`.

---

# v5.2.56 — REST ENGINE v1 PHASE 5 ALTERNATE DETERMINISTIC ALLOCATION CORRECTION

**Status:** SOURCE-QA; the narrow alternate deterministic allocation correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.56 correction scope

- Replaces independent per-obligation alternate-capacity tests with one deterministic replay for each contested clear-capacity record and uncertain higher-priority block.
- Removes only the higher-priority block from a copied allocation state while preserving every other actual block, reserved range and base restriction.
- Processes lower-priority obligations once in the accepted priority and full-fit/skip-fit order, consuming hypothetical capacity sequentially without creating public blocks or attachments.
- Enforces single-use ordinary bases and permits sequential multi-obligation replay only for an accepted shareable regular Weekly Rest base.
- Preserves the primary allocation, structured uncertainty timing, branch locality, indivisibility, confirmed positives and all accepted Phase 5 behavior.
- Adds T180–T183 and retains T100–T179.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.56

The correction evidence is recorded in `PHASE5_ALTERNATE_DETERMINISTIC_ALLOCATION_QA_REPORT_v5.2.56.md`.

---

# v5.2.55 — REST ENGINE v1 PHASE 5 STRUCTURED UNCERTAINTY-TIMING CORRECTION

**Status:** SOURCE-QA; the narrow structured uncertainty-timing correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.55 correction scope

- Replaces the v5.2.54 discovery-order/deadline timing shortcut with structured repayment uncertainty evidence.
- Retains accepted boundary-candidate chronologies and elapsed-range bounds without promoting them to factual timestamps.
- Uses remaining exact facts and optimistic legal bounds for structured no-candidate evidence; absence of candidates does not become certainty.
- Records possible completion instants for dependency-derived uncertainty so bounded downstream checks remain timing-specific.
- Propagates allocation dependency only when at least one possible higher-priority repayment package can complete by the contested capacity instant and the lower debt still passes the existing full-fit/materiality checks.
- Keeps deterministic primary allocation, priority, skip-fit, indivisibility, reserved ranges, base restrictions and confirmed positives unchanged.
- Adds T176–T179 and retains T100–T175.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.55

The correction evidence is recorded in `PHASE5_STRUCTURED_UNCERTAINTY_TIMING_QA_REPORT_v5.2.55.md`.

---

# v5.2.54 — REST ENGINE v1 PHASE 5 MULTI-OBLIGATION ALLOCATION-UNCERTAINTY CORRECTION

**Status:** SOURCE-QA; the narrow cross-obligation allocation-uncertainty correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.54 correction scope

- Records deterministic clear-capacity allocations without changing the accepted priority or skip-fit algorithm.
- Propagates REVIEW from a higher-priority obligation only when removing its specifically consumed block would leave a full, legal, pre-deadline package for a lower-priority obligation.
- Preserves all other occupied blocks, reserved Weekly Rest components, ordinary-base sharing restrictions, branch locality and indivisibility during the bounded alternate-feasibility check.
- Does not select the favourable history, confirm REVIEW repayment, allocate an alternate block or mark the lower obligation completed.
- Does not propagate when the higher debt cannot fit, both debts are confirmed, or the contested capacity is only usable after the lower debt deadline.
- Adds T172–T175 and retains T100–T171.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.54

The correction evidence is recorded in `PHASE5_MULTI_OBLIGATION_UNCERTAINTY_QA_REPORT_v5.2.54.md`.

---

# v5.2.53 — REST ENGINE v1 PHASE 5 FINAL REVIEW-ORDER CORRECTION

**Status:** SOURCE-QA; the narrow review-order and bounded-irrelevance correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.53 correction scope

- Applies the obligation-level uncertainty gate only after every RestInterval has been processed, eliminating order-dependent false `OVERDUE` results.
- Preserves confirmed Case A and provisional positive completion; preserves a Case B attachment relation while returning generic obligation `REVIEW` when earlier outcome-changing uncertainty remains.
- Treats an exact-start unresolved interval as irrelevant when its maximum pre-deadline duration is strictly shorter than the required indivisible compensation block; exact equality remains relevant.
- Treats an exact-end/unresolved-start interval as irrelevant when no legal package fits even from `obligation.createdAt` after reserved Weekly Rest components are excluded.
- Adds T166–T171 and retains T100–T165.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` and `main.tsx` remain byte-for-byte unchanged.

## Consolidated through v5.2.53

The final correction evidence is recorded in `PHASE5_FINAL_REVIEW_ORDER_QA_REPORT_v5.2.53.md`.

---

# v5.2.52 — REST ENGINE v1 PHASE 5 FINAL NONEXISTENT-GAP REVIEW CORRECTION

**Status:** SOURCE-QA; final no-candidate correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.52 correction scope

- Treats structured no-candidate unresolved boundaries such as `NONEXISTENT_GAP` as uncertainty unless remaining exact facts prove obligation-level irrelevance.
- Keeps v5.2.51 candidate/range possibility reasoning as the primary path.
- Preserves exact-start/exact-end/source-identity irrelevance controls and genuine complete-facts overdue.
- Never normalises an invalid wall time or invents a repayment timestamp, block, base or attachment.
- Preserves confirmed Case A, explicit Case B and the distinct `OPEN` provisional lifecycle.
- Adds T161–T165 and retains T100–T160.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` remains byte-for-byte unchanged.

## Consolidated through v5.2.52

The final gap-correction evidence is recorded in `PHASE5_FINAL_GAP_CORRECTION_QA_REPORT_v5.2.52.md`.

---

# v5.2.51 — REST ENGINE v1 PHASE 5 FINAL UNRESOLVED-BOUNDARY REVIEW CORRECTION

**Status:** SOURCE-QA; final narrow correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.51 correction scope

- Processes closed/history `REVIEW_REQUIRED` RestIntervals with null exact boundaries before the exact-boundary early skip.
- Uses accepted boundary candidates and elapsed-duration bounds to detect only deadline-relevant possible packages, including unresolved Case B.
- Never turns unresolved chronology into a confirmed block, base, attachment, Case A or Case B.
- Keeps proven-irrelevant unresolved ranges from erasing genuine complete-facts `CASE_C / OVERDUE`.
- Preserves confirmed Case A and the existing `OPEN` provisional lifecycle.
- Adds T156–T160 and retains T100–T155.
- Adds no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` remains byte-for-byte unchanged.

## Consolidated through v5.2.51

The final correction evidence is recorded in `PHASE5_FINAL_CORRECTION_QA_REPORT_v5.2.51.md`.

---

# v5.2.50 — REST ENGINE v1 PHASE 5 NARROW REVIEW-PROPAGATION CORRECTION

**Status:** SOURCE-QA; narrow correction is implemented and Phase 5 remains inactive at runtime.

## v5.2.50 correction scope

- Prevented a later clear late repayment from erasing relevant pre-deadline REVIEW uncertainty and producing a false strong `OVERDUE` result.
- Required complete factual coverage before a strong negative `CASE_C / OVERDUE` result.
- Limited REVIEW relevance to intervals that could change on-time completion; review starting only after deadline does not erase a certain breach.
- Preserved confirmed clear on-time completion, Case B, genuine complete-facts overdue, Phase 4 branch isolation and indivisible compensation rules.
- Added T151–T155 and retained T100–T150.
- Added no UI, storage, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` remains byte-for-byte unchanged.

## Consolidated through v5.2.50

The correction evidence is recorded in `PHASE5_CORRECTION_QA_REPORT_v5.2.50.md`.

---

# v5.2.49 — REST ENGINE v1 PHASE 5 COMPENSATION OBLIGATIONS / BLOCKS / ATTACHMENT

**Status:** SOURCE-QA; Phase 5 pure evaluator is implemented and remains inactive at runtime.

## Phase 5 scope

- Added branch-local `CompensationObligation` derivation only from counted reduced Weekly Rest components.
- Added exact Europe/London civil deadlines from the Phase 4 source fixed-week assignment.
- Added indivisible `CompensationBlock`, `AttachmentBase`, `AttachmentRelation` and per-obligation result models.
- Added bounded base/block layout alternatives for deadline Case A, unresolved Case B and overdue Case C.
- Added deterministic multiple-obligation priority with full-fit skipping, separate non-overlapping blocks and supported sharing only on a clear regular Weekly Rest base.
- Added conservative REVIEW propagation, fresh recomputation, convergence descriptors and T100–T150 focused coverage.
- Added no UI, storage, ledger, migration, warning aggregation or runtime integration. Phases 6, 7 and 8 remain not started.
- `App.tsx` remains byte-for-byte unchanged.

## Consolidated through v5.2.49

The complete Phase 5 evidence is recorded in `PHASE5_QA_REPORT_v5.2.49.md`.

---

# v5.2.48 — REST ENGINE v1 PHASE 4 FINAL REVIEW UNCERTAINTY CORRECTION

**Status:** SOURCE-QA; final narrow Phase 4 correction remains inactive at runtime.

## v5.2.48 correction scope

- Preserved current branch `REVIEW_REQUIRED` in the `prior qualifying rest + no confirmed rolling rest` path before any strong `AFTER_DUE` decision.
- Kept `REVIEW_ONLY` unconfirmed: it creates no rolling reset and no replacement anchor.
- Preserved genuine complete no-rest enforcement as `AFTER_DUE → VIOLATED`.
- Added focused T98 and T99 coverage; no Phase 1–3, fixed-week allocation, complete-RestInterval anchor, UI, storage, compensation or runtime integration change.
- `App.tsx` remains byte-for-byte unchanged and Phase 5 remains not started.

## Consolidated through v5.2.48

The final narrow-correction evidence is recorded in `PHASE4_FINAL_CORRECTION_QA_REPORT_v5.2.48.md`.

---

# v5.2.47 — REST ENGINE v1 PHASE 4 ALLOCATION-ANCHOR/COUNTING CORRECTION

**Status:** SOURCE-QA; narrow Phase 4 correction is implemented and remains inactive at runtime.

## v5.2.47 correction scope

- Separated rolling 144h qualification from fixed-week `COUNTED`/`ADDITIONAL` allocation roles, so a qualifying additional Weekly Rest resets rolling chronology.
- Anchored each external rolling restart at the complete factual `RestInterval` end; internal 24h/45h minimum-component boundaries and uninterrupted trailing surplus no longer move that anchor.
- Eliminated branches with redundant counted components in one fixed week while preserving distinct source alternatives as separate canonical branches.
- Added T91–T97 for the three independent-review blockers and updated affected Phase 4 regression fixtures without weakening their legal assertions.
- Added no compensation, debt, repayment, UI, storage, migration or runtime integration behavior. Phase 5 remains not started.
- `App.tsx` remains byte-for-byte unchanged.

## Consolidated through v5.2.47

The narrow-correction evidence is recorded in `PHASE4_CORRECTION_QA_REPORT_v5.2.47.md`.

---

# v5.2.46 — REST ENGINE v1 PHASE 4 LEGAL ALLOCATION SOLVER

**Status:** SOURCE-QA; Phase 4 solver is implemented and remains inactive at runtime.

## Phase 4 scope

- Added pure allocation branches over accepted Phase 2+3 `RestInterval` and `WeeklyRestComponentOption` inputs.
- Added exact counted/additional components, fixed-week assignments, two-consecutive-week evaluations and exact rolling 144h reset chronology.
- Preserved both eligible cross-week owners and both 69h orientations; materialized non-overlapping 69h and 90h back-to-back component ranges.
- Added explicit REVIEW, insufficient-history, pending and violation states with stable deterministic branch identities and source provenance.
- Added T51–T90 focused coverage for all locked Phase 4 rules.
- No compensation obligation, repayment, warning/UI aggregation, runtime integration, storage migration or Phase 5 behavior was added.
- `App.tsx` remains byte-for-byte unchanged and production runtime continues to use the established legacy Rest path.

## Consolidated through v5.2.46

The complete Phase 4 evidence is recorded in `PHASE4_QA_REPORT_v5.2.46.md`.

---

# v5.2.45 — PHASE 2+3 DIRECTIONAL REVIEW PROPAGATION CORRECTION

**Status:** SOURCE-QA; narrow Phase 2+3 correction remains inactive at runtime.

## Independent review finding and correction

- v5.2.44 propagated the missing-Finish reason from a known factual Start backward into the otherwise established rest ending at that Start.
- v5.2.45 separates reasons affecting the known Start boundary from forward-only uncertainty in the busy span.
- The incomplete-work diagnostic and Start-through-`asOf` barrier remain; no post-Start rest is derived.
- A clear 34h preceding rest retains `SINGLE_REDUCED`, and a clear >=45h preceding rest retains `SINGLE_REGULAR`.
- Ambiguous or unresolved Start boundaries remain REVIEW and cannot become confirmed candidates.
- `App.tsx` and `src/rest-engine/time.ts` are unchanged; no runtime wiring, storage change, allocation, debt or Phase 4 logic is introduced.

## Consolidated through v5.2.45

The correction evidence is recorded in `PHASE2_3_CORRECTION_QA_REPORT_v5.2.45.md`.

---

# v5.2.44 — REST ENGINE v1 COMBINED PHASE 2+3 CHRONOLOGY AND CANDIDATES

**Status:** SOURCE-QA; combined Phase 2+3 infrastructure is implemented and remains inactive at runtime.

## Phase 2+3 scope

- Added normalized `ActivityFact` inputs with explicit provenance, factual coverage, status, supersession, duplicate handling and review diagnostics.
- Added deterministic factual `RestInterval` derivation using exact Phase 1 instants, explicit `asOf`, maximal continuous chronology, open intervals and conservative barriers for incomplete or invalid work.
- Added bounded parametric Weekly Rest component options for exact 24h, 45h, 69h and 90h capability, including both 45h+24h orientations where a continuous interval crosses a Monday fixed-week boundary.
- Preserved candidate status only: no final weekly allocation, 144h solver, anti-double-counting consumption, compensation debt, UI wiring, storage writes or migration logic was added.
- The new modules are not imported by `App.tsx` or `main.tsx`; existing Rest Engine, Pay, End Week, archive, storage and UI behavior remains on the v5.2.43 runtime path.

## Consolidated through v5.2.44

The complete combined Phase 2+3 evidence is recorded in `PHASE2_3_QA_REPORT_v5.2.44.md`.

---

# v5.2.43 — REST ENGINE v1 PHASE 1 TIME FOUNDATION

**Status:** SOURCE-QA; Phase 1 infrastructure is implemented and remains inactive at runtime.

## Phase 1 scope

- Added an isolated `src/rest-engine` time foundation using explicit `Europe/London` timezone data.
- Added typed UTC instant, entered wall date/time, offset, provenance and ambiguity/review representations.
- Added exact elapsed-time arithmetic, London civil formatting, fixed legal-week boundaries and Sunday 24:00 boundary support.
- Added deterministic tests for the historical 21:00 regression, both 2026 DST transitions, exact 144h ordering, device-timezone independence, fold/gap handling and legal week/deadline boundaries.
- The new module is not imported by `App.tsx`; current Daily Rest, Weekly Rest, compensation, End Week, archive, Pay and UI behaviour remains on the v5.2.42 runtime path.

## Consolidated through v5.2.43

The complete Phase 1 evidence is recorded in `PHASE1_QA_REPORT_v5.2.43.md`.

---

# v5.2.42 — END WEEK PAY CONTEXT CARRY-FORWARD FIX

**Status:** SOURCE-QA; focused regression prepared, phone End Week verification required.

## Confirmed defect

A real v5.2.41 Saturday End Week reproduction showed an unchanged active Gross Only profile (`ARC → Profile 1`, weekday £18.00, Saturday £20.00, Sunday £21.00) switching immediately after End Week to PAYE estimate with £14.00 / £21.00 / £21.00 while the visible active-profile label still remained `ARC → Profile 1`. Those post-End-Week values exactly match `initialSettings`.

The defect is in the transition into the next pay period, not in the Pay formula and not in a user profile edit. `loadSavedWeekDataOrBlank()` deliberately returns `initialSettings` when the target week has no persisted week/archive record. `openNextPayPeriod()` then used that fallback as real week Settings. Because an unresolved next-week profile id did not clear the existing active-profile state, the UI could display the old profile name while calculations used unrelated default PAYE rates.

## Why it surfaced now

The vulnerable End Week path is historical and is present in the preserved v5.1.11 source as well as v5.2.36–v5.2.41. The recent v5.2.37–v5.2.41 patches did not directly change this Pay Profile / End Week Settings path. The defect is state-dependent: it only takes the bad branch when the target next pay week has no persisted pay-week record. If the target week already exists, its own saved Settings are loaded and the reset is not seen. Therefore this is classified as a **latent historical defect newly triggered by a missing next-week record**, not as a proven direct regression introduced by one of the September Start/Rest/UI patches. The exact historical reason earlier target weeks already had a persisted context cannot be reconstructed from source alone; that is not guessed.

## v5.2.42 correction

- End Week first checks whether the target next pay week already has a persisted week/archive record.
- If the next week does **not** exist, it inherits the current factual pay context: current `settings` plus current `activePayProfileId`.
- Therefore Gross Only/PAYE mode, weekday/Saturday/Sunday rates, pension mode, overtime, allowances, bonuses and the active profile continue unchanged into a genuinely new pay week.
- If the target week **already exists**, its own saved Settings/profile context remains authoritative and is not overwritten. This protects intentionally prepared future weeks and historical saved data.
- KM carry and Working tomorrow? target-day behavior are unchanged.
- Pay formulas, Rest Engine, Weekly Rest, compensation, archive semantics, Start/KM provenance, colours and general UI are unchanged.

## Mandatory regression

1. Current week: Gross Only, active profile `ARC → Profile 1`, rates 18 / 20 / 21; next week absent. End Week must open next week still Gross Only, same active profile, same rates.
2. Reload after the transition must retain that inherited context because the normal week persistence effect saves the new week with the inherited Settings/profile id.
3. PAYE current context must likewise remain PAYE when next week is absent.
4. If next week already has a saved different pay context, End Week must preserve that saved next-week context rather than overwrite it.

## Consolidated through v5.2.42
...
Phone verification of v5.2.40 exposed the second half of the same Daily Start presentation defect. v5.2.40 correctly stopped promoting a retained cross-day 9h helper into the main Start field, but `getSuggestedStartTimesForDay` still nulled the regular 11h boundary whenever that boundary fell on the previous calendar day. The result was an empty main Start field plus `9h option: ...`, even though the driver had already completed 11h+ rest and the regular 11h boundary remained the correct primary proposal.

### v5.2.41 correction

- Within the existing less-than-24h Daily Start suggestion window, the regular 11h boundary is retained even if it falls on the previous calendar day.
- 11h remains the normal primary Start proposal and renders with `from 11h rest`.
- 9h remains the secondary helper when allowed, including the protected historical cross-day helper behavior.
- 9h may still become primary only when 11h is genuinely unavailable because the previous duty exceeded 13h.
- The existing 24h cutoff remains unchanged; after 24h+ this is no longer treated as a Daily Start suggestion situation.
- No Daily Rest thresholds, reduced-rest allowance, Split Rest, Weekly Rest, compensation, KM, Pay, archive/navigation, storage, bonus-row or other UI behavior is changed.

## Consolidated through v5.2.41

# v5.2.40 — DAILY START PRIMARY 11H/9H OWNERSHIP FIX

**Status:** SOURCE-QA; narrow source sanity checked, no full QA run.

## Narrow runtime change

Phone review of v5.2.39 exposed an invalid Daily Start presentation on a later calendar day: when the regular 11h boundary had already fallen on the previous calendar day, `getSuggestedStartTimesForDay` correctly removed that old 11h time from the current-day Start field but intentionally retained the historical 9h helper. `getPrimarySuggestedStart` then incorrectly promoted that retained 9h helper into the main Start proposal. The UI therefore showed the same old 9h boundary twice (`from 9h rest` plus `9h option`) even though 11h was not genuinely unavailable.

### v5.2.40 correction

- 11h remains the normal primary Daily Start proposal when it belongs to the selected day.
- 9h may become the primary Start proposal only when 11h is genuinely unavailable because the previous duty exceeded 13h.
- If the 11h boundary merely fell on the previous calendar day, Start stays neutral instead of falsely promoting the retained historical 9h helper.
- The protected historical rule that the 9h helper may remain visible across the calendar-day boundary is unchanged.
- No Daily Rest thresholds, reduced-rest allowance, Split Rest, Weekly Rest, compensation, KM, Pay, archive/navigation, storage, bonus-row or PWA behavior is changed.

## Consolidated through v5.2.40

# v5.2.39 — WEEKLY REST ENDED CARD OWNERSHIP FIX

**Status:** SOURCE-QA; intentionally not regression-tested at user request.

## Narrow runtime change

Phone review of v5.2.38 confirmed that removing the long `Weekly rest ended ...` helper from only the Start column stabilized Start/Finish alignment, but the Weekly Rest inline card did not actually consume the already-computed `primaryHelp`. As a result, after the 45h endpoint had passed and before factual Start, the card still rendered `45h Start: ...` instead of the intended completed-context line.

### v5.2.39 correction

- `WeeklyRestInlineCard` now uses the existing `primaryHelp === weeklyRestEnded` state.
- Before the endpoint: the card remains `45h Start: [day/time]`.
- After the endpoint and before factual Start: the card renders `Weekly rest ended [day/time]`.
- The long ended text is NOT returned below the Start field, so Start/Finish alignment remains unchanged.
- No Rest Engine, Weekly Rest calculation, End Week/candidate ownership, compensation, KM, Pay, navigation, archive, bonus-row, storage, dependency or PWA-name behavior is changed.
- No tests were added or run for this version, by explicit user decision; verification is by narrow source inspection and phone QA.

## Consolidated through v5.2.39

# v5.2.38 — MOBILE START-ROW STABILITY / BONUS-ROW FIT

**Date:** 6 September 2026  
**Status:** SOURCE-QA; v5.2.38-r1 QA-only correction prepared; not QA ACCEPTED or stable.

## Reason
Phone review of v5.2.36 showed a narrow-layout annoyance after a completed 45h Weekly Rest target: the duplicate `Weekly rest ended ...` flow hint lived only under the Start field, making the Start column taller and visually pushing the Finish field out of alignment. The same phone review also showed the bonus selector clipping short labels such as `ADR` because the quantity field consumed too much horizontal space.

## Exact changes
### Start / Finish row stability
- When the 45h Weekly Rest target is already behind the selected day, the Start field no longer renders the duplicate `Weekly rest ended ...` flow hint inside only the Start column.
- The Weekly Rest card immediately below remains the owner of that context, so information is not invented or recalculated.
- Start and Finish fields therefore keep the same row alignment in this already-completed-target scenario.
- Normal daily `from 11h rest` / `from 9h rest` inline provenance remains unchanged.
- No Weekly Rest calculation, target time, candidate ownership or Rest Card semantics change.

### Bonus draft row fit
- Bonus selector receives the flexible remaining width.
- Quantity field is reduced from 80px to 56px, appropriate for the normal 1–2 digit manual count workflow.
- Add button stays 88px.
- Bonus selector font is 14px with narrower horizontal padding and `minWidth: 0` so native mobile select rendering has more usable text space.
- Quantity input is centered with tighter horizontal padding.
- Bonus calculation, stored quantity, custom bonus names and Add behavior are unchanged.

## Carried v5.2.37 correctness fixes
- Week Preview factual date-aware rest chronology and exact 24h/reset correction remain intact.
- Legacy compensation repayment continues to reuse the one-rest/one-debt guarded helper.

## Intentionally protected / unchanged
- Pay Engine, KM provenance, Daily/Weekly Rest calculations, Split rules, compensation amounts/deadlines.
- End Week, Saturday Save & Next, Working tomorrow, archive/navigation and backup.
- Weekly Rest card placement/ownership beyond removing this one duplicate Start-column hint.
- Six-cycle deadline wording/colour remains a separate open semantic UX item.
- General mobile density is not redesigned.

## Regression evidence at source-QA creation
- backup/restore round-trip + stale-state replacement + rollback safety: **PASS**.
- Weekly Rest factual timeline: **PASS**.
- End Week next-day intent: **PASS**.
- compensation creation: **PASS**.
- compensation repayment / one-rest-one-debt guard: **PASS**.
- v5.2.20 UX/Weekly Rest validation: **PASS**.
- v5.2.21 Rest/archive: **PASS**.
- v5.2.22 Weekly Rest/current-day: **PASS**.
- v5.2.27 road-test ownership/workflow: **PASS**.
- v5.2.31 lifecycle/counter: **PASS**.
- v5.2.32 Start provenance backward compatibility: **PASS**.
- v5.2.33 chronology/candidate ownership: **PASS**.
- v5.2.36 Start helper / KM provenance: **PASS**.
- `v5.2.37 Week Preview factual rest chronology regression`: **PASS**.
- `v5.2.38 mobile Start-row / bonus-row regression`: **PASS**.
- release/version consistency: **PASS**.
- isolated TypeScript check: **PASS**.
- esbuild-backed v5.2.23 / v5.2.24 / historical v5.2.26 contract runner: **BLOCKED in this container because `esbuild` is unavailable after incomplete npm dependency installation**; no application assertion failure is claimed from those runs.
- Fresh Vite build and real-App Chromium remain **BLOCKED in this container by incomplete npm dependency installation**; no application assertion failure is claimed.

## v5.2.38-r1 — QA-only correction after independent Windows run

Runtime remains **v5.2.38**. No production source, Pay/Rest/KM/business logic, CSS, manifest, service-worker cache, storage or dependency version is changed by r1.

### Independent Windows QA result on original v5.2.38
- clean `npm ci --no-audit --no-fund`: **PASS**;
- TypeScript: **PASS**;
- production Vite build: **PASS**;
- current regression suites through v5.2.38: **PASS** except the known historical v5.2.26 stale expectation;
- v5.2.37 chronology / compensation guard: **PASS**;
- v5.2.38 source UI checks: **PASS**;
- dynamic real-App Start-provenance / legacy factual Start suite: **PASS (4/4)**;
- production source changed during QA: **NO**; GitHub touched: **NO**.

Two blockers were classified as QA defects rather than demonstrated application defects:
1. `scripts/v5-2-30-real-app-browser-regression-test.mjs` used static August 2026 dates. Once the real date moved into September, startup correctly followed the actual current workflow date, so the intended Sunday 15h/21h/24h state was not selected.
2. `scripts/v5-2-25-weekly-rest-ui-contract-test.mjs` retained the pre-v5.2.33 expectation that a normally saved previous week can manufacture Weekly Rest proposal ownership without End Week/archive evidence.

### r1 corrections
- The v5.2.30 real-App fixture is now date-stable: it derives the current payroll Saturday at runtime, derives all fixture dates relative to it, forces the browser to UTC as before, and explicitly selects the fixture Sunday for 15h/21h/24h plus the fixture Saturday for Save & Next / End Week. Existing behavioral assertions are retained.
- The historical v5.2.26 contract now asserts the current protected ownership rule instead of the superseded one: open/saved previous-week facts remain chronology only and do not replace a genuine stored proposal candidate; when that previous week has closed/archived End Week evidence, its newer factual anchor may backfill and win.
- No harness workaround changes application source or weakens the current v5.2.33 candidate-ownership contract.

### r1 QA status at packaging
- both corrected scripts pass Node syntax validation;
- dependency-free v5.2.33, v5.2.37, v5.2.38 and release/version regressions remain PASS in the packaging environment;
- full esbuild-backed v5.2.26 and real-App browser execution still require the independent Windows environment with installed dependencies/Chrome and are **not claimed as passed by this packaging step**.

## Historical consolidated record through v5.2.38
This document carries forward the verified v5.2.37 and earlier history below.
...
**Date:** 6 September 2026  
**Status:** SOURCE-QA; not QA ACCEPTED or stable.

## Reason
Inspection of the carried backlog found two correctness risks in v5.2.36 that were still present in production source. Week Preview used its own adjacent-day/time-of-day rest reducer instead of the date-aware factual chronology used by the main Rest Engine. That made exact cross-date 24h appear as 0h, lost rest across Off/Holiday gaps, could miss the previous pay-week Finish anchor, and used the displayed reduced-rest total as if it were the live allowance counter. Separately, the newer timeline compensation path already prevented one continuous rest from repaying two debts, but the legacy compensation effect still duplicated repayment logic without the same rest-reuse guard.

## Exact changes
### Week Preview chronology consistency
- Added `getWeekPreviewRestSummary(currentDays, archive)`.
- Week Preview now derives its Rest Snapshot from `buildWeeklyRestTimelineDays`, the same factual date-aware merged chronology used by the main Rest Engine.
- Rest duration uses absolute date+time minutes, not only clock-time subtraction.
- Off/Holiday/untouched placeholder days no longer erase the previous factual Finish anchor.
- Saved prior pay-week facts can participate in the Sunday/current-week Rest Snapshot through the existing archive -> saved week -> live week precedence.
- The displayed total number of reduced rests is now separate from the reduced-rest allowance counter used for legality/lifecycle decisions.
- A factual `>=24h` Weekly Rest resets the internal reduced-rest cycle before later Daily Rest classification.
- Exact 23h59 does not reset; exact 24h does.
- Split Rest remains separate and does not consume an ordinary reduced-rest allowance.
- `WeekViewModal` no longer runs its own adjacent-day `previewWeek.reduce` mini Rest Engine.

### Compensation one-rest / one-debt guard
- The legacy compensation effect now reuses `completeEarliestEligibleWeeklyCompensation(...)`, the same guarded repayment helper already used by the factual timeline path.
- Re-editing Start while the same continuous rest is still in progress cannot spend that one rest on a second compensation debt.
- Earliest eligible obligation, deadline, chronology and indivisible `9h base + debt` repayment rules remain unchanged.

## Intentionally protected / unchanged
- Main Rest Card chronology and Weekly Rest recognition thresholds.
- 11h / 9h Daily Rest thresholds and Split Rest semantics.
- Six-cycle ownership and End Week candidate ownership.
- Pay Engine, guaranteed hours, OT, weekend rates, pension, bonuses, Night out and tax estimate.
- Start helper presentation and v5.2.36 Start/KM provenance rules.
- Archive, backup/restore, Working tomorrow, Saturday Save & Next and navigation semantics.
- Weekly Rest proposal/card ownership and mobile layout are not redesigned in this patch.
- The separate phone layout issue where Start helper text changes Start/Finish row alignment is not included here.
- Bonus-row mobile width/font cleanup is not included here.

## Regression evidence at source-QA creation
- Global TypeScript syntax/type check using the project tsconfig with an isolated empty external typeRoot: **PASS**.
- `v5.2.37 Week Preview factual rest chronology regression`: **PASS**.
  - exact cross-date 24h;
  - Off-day gap;
  - 3 reduced rests -> exact 24h reset -> valid next 9h reduced rest;
  - 23h59 no-reset boundary;
  - Split Rest non-consumption;
  - cross-pay-week previous Finish -> Sunday Start classification.
- `timeline-compensation-repayment-test`: **PASS**, including one-rest/one-debt Start-edit regression and source integration guard for both timeline and legacy repayment paths.
- backup round-trip: **PASS**.
- weekly-rest timeline: **PASS**.
- End Week intent: **PASS**.
- compensation creation: **PASS**.
- v5.2.20: **PASS**.
- v5.2.21: **PASS**.
- v5.2.22: **PASS**.
...
- v5.2.33: **PASS**.
- v5.2.36: **PASS**.
- esbuild-backed v5.2.23/v5.2.24/v5.2.26 and fresh Vite build/real-App Chromium are **BLOCKED in this container by incomplete dependency installation**, not by an observed application assertion failure.
- Full stable promotion therefore is not claimed.

## Consolidated through v5.2.37
This document carries forward the verified v5.2.36 history below. Newer explicit v5.2.37 decisions supersede older conflicting Week Preview/legacy compensation behavior only within the narrow scope described above.
...
## Reason
Phone review of v5.2.35 found two narrow regressions/UX mismatches. First, the mobile Start fit rule made ordinary daily HH:MM proposals too small and moved the normal `from 11h rest` / `from 9h rest` provenance outside the Start field. Second, an automatically carried Start KM could turn factual/black merely because Finish time or other work data existed, even though time completion does not confirm distance.

The product rule is minimal-input-first: pay verification is primarily time-driven. A valid paid Work day may have Start/Finish time and no KM entry at all. KM is optional supporting data and must not become factual from unrelated time/pay actions.

## Exact changes
### Start proposal presentation
- Ordinary daily HH:MM Start proposals keep the normal 24px emphasis on narrow screens.
- `from 11h rest` and `from 9h rest` are restored inside the Start field, matching the established compact field pattern used by suggested Start KM.
- The existing helper below Start continues to own the alternative/explanation, including `9h option: HH:MM`, `11h rest unavailable`, and reduced-rest-limit wording.
- Longer weekday/time Weekly Rest proposals such as `Sun 13:35` keep the v5.2.35 compact 20px narrow-screen fit.
- Long contextual Weekly Rest helper text such as `Weekly rest ended Sun 13:35` keeps separate mobile flow space and must not overlap the Start field.
- Factual/user-entered Start remains 24px. Finish presentation remains unchanged.

### KM provenance / minimal-input rule
- Added explicit Start KM provenance: `user`, `suggestedCarry`, or `confirmedCarry`.
- The last factual Finish KM is the only carry-forward anchor. A Start KM by itself never becomes the next carry anchor.
- An automatically carried Start KM remains suggested/grey with its source helper through Start time, Finish time, Save & Next, bonuses, Night out, Split break, and other non-KM actions.
- Entering Finish KM confirms an untouched carried Start KM for that KM run; the new Finish KM becomes the next factual carry-forward anchor.
- Manually typing/changing Start KM marks that Start KM factual/user-entered immediately.
- If no Finish KM is entered for one or more paid Work days, the last factual Finish KM continues to be offered on later Work days and across pay-week close/open.
- Save & Next / End Week remain valid without KM. No invented distance is required merely because the driver worked, trained, waited, repaired a vehicle, or otherwise had a paid non-driving day.
- Legacy v5.2.35-style carried Start KM records without provenance are treated as suggested when they still equal the last factual Finish KM and have no Finish KM.

## Intentionally protected / unchanged
- Pay Engine, rates, tax, pension, bonuses and time-based pay calculations.
- Daily Rest thresholds, reduced-rest counter, Split Rest rules and Rest Card facts.
- Weekly Rest chronology, End Week candidate ownership, 45h/24h logic, six-cycle logic and compensation model.
- Week Preview logic, archive/navigation behavior, Working tomorrow flow and calendar ownership.
- PWA installed name remains `Driver Pay App v5`; runtime identity is v5.2.36 and service-worker cache remains version-specific.
- Storage keys and existing backup container/schema remain backward compatible; the new optional Start KM provenance field is additive day-record metadata.
- v5.2.35 mobile empty-state density changes remain unchanged.

## Required acceptance scenarios
1. Daily 11h proposal: normal HH:MM at 24px; `from 11h rest` inside Start; `9h option` remains below.
2. 11h unavailable: 9h HH:MM primary at 24px; `from 9h rest` inside Start; existing unavailable/limit helper remains below.
3. Weekly proposal `Sun 13:35`: fits at 360px/390px without clipping.
4. `Weekly rest ended Sun 13:35`: separate readable helper with no overlap at 360px/390px.
5. Factual Start `07:00`: remains 24px.
6. Carried Start KM + Start time + Finish time: Start KM remains grey/suggested and helper remains visible.
7. Carried Start KM + bonuses/Night out/Split without Finish KM: remains suggested.
8. Carried Start KM + Finish KM: Start KM becomes factual/confirmed and new Finish KM becomes next carry anchor.
9. Manual Start KM edit: Start KM becomes factual/user-entered.
10. Two or more paid Work days with Start/Finish times but no KM: original last factual Finish KM continues to be suggested.
11. End Week with no new Finish KM: next pay period still suggests the same last factual Finish KM.
12. No KM is required to Save & Next or close a paid Work day/week.

## QA evidence at package creation
- TypeScript (`tsc --noEmit`): **PASS** in the packaging environment.
- `v5.2.20 UX / weekly-rest validation regression`: **PASS after intentional KM assertion update**. The older assertion tied suggested Start KM to `dayHasDestructiveWorkData`; v5.2.36 explicitly supersedes that product rule, so the test now checks Finish-KM/provenance ownership instead.
- `v5.2.36 Start helper / KM provenance regression`: **PASS**.
- v5.2.32 Start provenance regression: **PASS**.
- v5.2.33 saved-week chronology / candidate-ownership regression: **PASS**.
- Release/version consistency: **PASS** after final version/documentation sync.
- v5.2.24 esbuild-backed regression: **BLOCKED in the packaging environment because project dependencies are intentionally absent from the clean source ZIP**; no application assertion failure is claimed from that attempt.
- Production build / real-App browser / physical phone acceptance: **not claimed yet**; targeted independent QA remains required.
- The known pre-existing v5.2.26 candidate-selection contract conflict is outside this v5.2.36 scope and must not be modified merely to obtain a green full suite.

## Final Source-QA r1 packaging evidence
- Targeted rendered QA passed at 360px, 390px and desktop: ordinary 11h and 9h Start proposals remain 24px with their provenance inside the Start field; the 11h-unavailable helper remains separate; contextual weekday/time proposals use 20px only where required; factual Start remains 24px.
- KM-01 through KM-09 passed in the real application. Suggested carried Start KM remains suggested through time entry, Split, Night out, bonuses, Save & Next and End Week; only Finish KM confirms it and advances the factual anchor. No-KM paid Work days and pay-week rollover retain the last factual Finish KM.
- The original `test:browser:v5.2.32-start` failure was a time-dependent harness defect: it selected the real current Monday, so expected 05:00/03:00 suggestions were stale when run later that day. The QA-only correction in `scripts/v5-2-32-start-provenance-real-app-test.mjs` selects the following pay week. Assertions remain unchanged; the browser regression subsequently passed all four assertions.
- TypeScript, v5.2.20, v5.2.31, v5.2.32, v5.2.33, v5.2.36, release-version, backup and production build all passed in the final targeted QA run.
- Rendered `Weekly rest ended ...` remains **BLOCKED**: no dedicated deterministic real-App fixture was created. This package remains **SOURCE-QA**, not stable or QA ACCEPTED, pending phone road test and remaining acceptance decisions.
...
## Reason
Narrow mobile layouts could clip short proposed Start times, overlay contextual Start hints, and use excessive vertical space for empty Work-day states. Windows-installed PWA shortcuts also exposed the patch runtime version rather than the stable v5 product identity.

## Exact changes
- On narrow screens only, a proposed Start uses a compact proposal presentation while factual Start values remain 24px.
- On narrow screens only, a Start hint receives its own layout space instead of overlaying the Start input.
- Empty Day Summary and empty mini-stat presentation are compacted on narrow screens without removing labels, rows, sections, or changing their order.
- The manifest `name` and `short_name` are fixed to `Driver Pay App v5`; runtime version, HTML title, manifest description, and service-worker cache identity remain v5.2.35.

## Intentionally protected / unchanged
- Start proposal generation, Weekly Rest logic, Rest Engine, validation, ownership semantics, stored values, storage keys, backup schema/data, and persistence.
- Kilometres, Night out / Split break, Bonuses, Day Summary, Hours, Overtime, KM, Save & Next, Week View, and existing section order.
- Desktop presentation, settings, modals, icons, manifest start URL/scope/id/display mode, and service-worker version-specific cache identity.

## Targeted QA evidence
- TypeScript: **PASS**.
- Release/version consistency: **PASS**.
- Production build: **PASS**.
- v5.2.24 targeted regression: **PASS**.
- v5.2.26 weekly-rest candidate-selection assertion: **FAIL / pre-existing**. The assertion expects `2030-08-10` and receives `2030-08-03`; this is outside the locked v5.2.35 scope and was not changed.
- Browser fixture: produced through the real application flow (saved Friday finish at 16:35, End Week, weekly-rest selection), not by DOM text injection.
- Mobile 360px proposed Start `Sun 13:35`: **PASS** — fully visible without right-side clipping; Start field 130.5px wide, proposal font 20px.
- Mobile 360px `Weekly rest ended Sun 13:35` helper: **PASS** — fully readable in separate vertical space, with a 4px gap below the 58px Start input and no overlap.
- Mobile 390px proposed Start `Sun 13:35`: **PASS** — fully visible without right-side clipping; Start field 145.5px wide, proposal font 20px.
- Mobile 390px `Weekly rest ended Sun 13:35` helper: **PASS** — fully readable in separate vertical space, with a 4px gap below the 58px Start input and no overlap.
- Factual Start `07:00`: **PASS** — retains the normal 24px presentation and 58px field height at narrow widths.
- Desktop Start/Finish regression control: **PASS** — at 1024px both fields remain 194px by 58px at 24px; the mobile-only layout is inactive.
- Mobile density: **PASS**. Empty Day Summary and empty mini-stats are compacted only below the narrow-screen breakpoint.
- Required sections preserved and order unchanged: **PASS** — Kilometres, Night out / Split break, Bonuses, Day Summary, Hours, Overtime, KM, Save & Next, and Week View remain present in their existing order.
- Temporary browser-storage cleanup: **BLOCKED / partial (QA-environment note only)**. The dedicated QA tab was closed; the application has no in-product local-data clear action, and direct browser-storage mutation was not used.

---

# v5.2.34 — START CONTEXT PLACEHOLDER FIT — VISUAL-ONLY PATCH

## Trigger
Phone road testing showed that long contextual text displayed inside the empty Start field could be visually squeezed/clipped in the two-column Shift layout.

## Narrow runtime change
- `TimeRow` now reduces the display font only while an empty field is showing a long placeholder.
- Short placeholders keep the existing large presentation.
- As soon as a factual/user-entered value exists, the normal 24px time presentation is restored.

## Explicitly unchanged
No intentional change to Weekly Rest chronology, candidate ownership, compensation, Split Rest, KM, Pay Engine, archive/navigation, storage, Rest status colours, helper ownership, or user-facing wording.

## QA status
Release/version consistency and the v5.2.33 saved-week chronology/candidate-ownership regression are required to remain green. Phone visual confirmation on the narrow layout remains required before stable promotion.
...
## Trigger
Read-only inspection of v5.2.32 found a factual cross-pay-week consistency defect when the calendar/pay week rolled forward without the user pressing `End Week`.

The daily Rest Card could recover the previous factual Finish from the normal saved previous week, while the Weekly Rest timeline and reduced-daily-rest counter built their chronology only from current days plus archive records. The two paths could therefore classify the same real history differently.

A second confirmed ownership defect existed in the legacy candidate backfill: an immediately previous normal saved week could manufacture a Weekly Rest candidate even when there was no evidence that `End Week` had been used.

## Narrow runtime fix
- Normal `driverApp_week_*` saved weeks now participate in the factual Weekly Rest / reduced-daily-rest chronology even when they were not closed with `End Week`.
- Same-date precedence is explicit: archive baseline -> normal saved week -> current live week.
- The pre-selected-day Weekly Rest ownership snapshot caps saved-week history at the preceding calendar day so an autosaved copy of the selected day cannot leak a newly entered Start back into the pre-Start snapshot.
- Normal saved history remains factual chronology only. It does **not** become Weekly Rest proposal intent.
- Legacy previous-week candidate backfill now requires evidence of `End Week`: either the closed-week index or an existing archive record. A normal saved week by itself is insufficient.

## QA / release support included with the same fix
- Added `scripts/v5-2-33-chronology-saved-week-regression-test.mjs` with behavioral rollover, 24h cross-boundary, source-precedence and candidate-ownership scenarios plus production-source binding guards.
- Active Node test runners that derived Windows paths directly from `new URL(import.meta.url).pathname` now use `fileURLToPath(import.meta.url)`. This changes QA harness portability only; it does not change runtime application behavior.
- Added a dependency-free release/version consistency test for source-package identity.

## Explicitly unchanged
No intentional change to:
- UI layout, cards, spacing, colours or responsive presentation;
- user-facing wording or translations;
- Pay Engine, rates, tax, pension, bonuses or summaries;
- KM calculation, KM carry-forward or KM provenance;
- Week Preview presentation/markers;
- daily-rest thresholds or Split Rest rules;
- Weekly Rest 45h/24h proposal presentation;
- compensation creation/repayment model;
- End Week dialog flow, Working tomorrow, archive correction or navigation semantics;
- storage keys or backup format.

## QA status in this source package
Dependency-free regression suites through v5.2.33 pass locally, including the protected weekly-rest timeline, End Week intent, compensation, v5.2.20/v5.2.21/v5.2.22/v5.2.27/v5.2.31/v5.2.32 and new v5.2.33 checks.

A full `npm ci` attempt in the packaging environment could not complete because npm registry downloads failed with `EAI_AGAIN`. Therefore TypeScript, esbuild-dependent suites, fresh Vite build and real-App Chromium acceptance remain mandatory independent QA before stable promotion.

**Status:** SOURCE-QA candidate; not yet promoted to stable.
...
**Scope:** runtime correctness only. The protected UI colour palette is intentionally **not changed** in this release.

#### START-003 — Typed Start provenance is factual
A Start typed by the user is factual immediately, even when its numeric value exactly equals the current 9h/11h suggestion.
Changing focus to Finish/KM/another field must not turn that factual Start back into a visual proposal.
Value equality is not evidence of proposal ownership.

#### REST-005 — Reduced daily-rest allowance follows the rest cycle
The three reduced daily rests are counted between factual weekly rest periods.
The counter does not reset merely because the Sunday→Saturday pay-week changes.
A factual reduced or regular weekly rest (24h+) completed by a later real Start resets the allowance cycle.

The counter must use the same effective daily-rest classification as the Rest Card:
- a >13h previous shift can make the next >=9h daily rest consume a reduced-rest allowance under the established app rule;
- a valid Split Rest does not consume the reduced allowance;
- after three reduced daily rests, another reduced-rest attempt is a violation/red state until a weekly rest resets the cycle.

#### REST-006 — Split Rest explanation stays with the factual Rest Card
When the factual Rest Card is classified as Split rest, it must also state that it does not count as a 9h reduced rest.

#### UX-006 — Complete is not automatically archive-like
Entering enough data to make a day complete does not by itself turn the current active day grey/archive-like.
Archive-like visual treatment may begin when the day is genuinely past in workflow/lifecycle, including:
- explicit Save & Next;
- End Week / closed week;
- real calendar passage into a later day;
- a factual Start on a later day.

#### NAV-003 — Current week means active workflow week
`Go to current week` is shown when the selected non-hard-archive week differs from the application's active workflow week.
It must not appear merely because the active app week is ahead of the device's current calendar payroll week.

#### Colour freeze
No Rest Card, traffic-light, Work-day, warning, split-rest, archive, or other semantic colour value is changed by v5.2.31.
Colour work remains a separate future task.
...
## Trigger
Real-App Chrome QA on v5.2.31 reproduced a concrete backward-compatibility defect for persisted records created before `startEntrySource` existed.

Confirmed runtime results on v5.2.31:
- manual Start exactly equal to the current 11h suggestion: PASS;
- manual Start exactly equal to the current 9h suggestion: PASS;
- legacy factual Start without `startEntrySource`, equal to the 11h suggestion: FAIL;
- legacy factual Start without `startEntrySource`, equal to the 9h suggestion: FAIL.

Observed defect: the legacy factual Start was present in persisted data but disappeared from the input value and returned as suggestion/placeholder state after load.

## Root cause
v5.2.31 introduced explicit provenance (`user` / `acceptedSuggestion`) but legacy records were sanitized with `startEntrySource: undefined`. Draft-detection code treated any provenance other than `user` as a possible accepted suggestion when the Start value equalled the current 9h/11h proposal.

## Narrow fix
- During sanitization, a legacy record with a non-empty stored Start and no valid `startEntrySource` is migrated to `startEntrySource: "user"`.
- Auto-accepted suggestion draft detection now requires the explicit provenance `startEntrySource === "acceptedSuggestion"`; missing/unknown provenance is never sufficient to hide factual Start data.
- No reduced-rest, weekly-rest, split-rest, archive, navigation, colour, wording, or translation logic is changed by this fix.

## Regression protection
Added:
- `scripts/v5-2-32-start-provenance-backward-compat-regression-test.mjs`
- `scripts/v5-2-32-start-provenance-real-app-test.mjs`
- npm script `test:v5.2.32`
- npm script `test:browser:v5.2.32-start`

Mandatory acceptance before phone road test:
1. exact manual 11h suggestion remains factual after blur — PASS required;
2. exact manual 9h suggestion remains factual after blur — PASS required;
3. legacy factual 11h-equal Start without provenance remains visible factual Start — PASS required;
4. legacy factual 9h-equal Start without provenance remains visible factual Start — PASS required;
