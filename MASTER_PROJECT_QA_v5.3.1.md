...
**Status:** SOURCE-QA; narrow correction for invalid-branch public warning and compensation presentation. Final executed evidence is in `PHASE8_BRANCH_WARNING_PRESENTATION_FIX_QA_REPORT_v5.3.1.md`. Physical phone acceptance remains pending.

## Consolidated through v5.3.1

- When a non-VIOLATED allocation branch survives, only surviving branches set public debt, compensation deadline, warning severity and compensation items. VIOLATED branches remain in diagnostic `branchWarnings`.
- A pure Weekly Rest violation with no applicable compensation obligation does not create an impossible-compensation item. The RED Weekly Rest warning remains visible in the Weekly Rest presentation.
- App.tsx, main.tsx, Pay Engine, daily rest, migration, Phase 1–6 legal modules and UI design remain unchanged from v5.3.0.
- T278–T283 cover the correction while T246–T277 and prior regression remain in place.
