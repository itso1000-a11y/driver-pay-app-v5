...
type WeeklyRestCandidate = { closingSaturdayISO: string; finishAbs: number; completedWorkCycles?: number };
...
function readWeeklyRestCandidate(): WeeklyRestCandidate | null {
  ...
  try {
    ...
    if (typeof parsed.closingSaturdayISO !== "string" || typeof parsed.finishAbs !== "number") return null;
    return {
      closingSaturdayISO: parsed.closingSaturdayISO,
      finishAbs: parsed.finishAbs,
      completedWorkCycles: Number.isFinite(Number(parsed.completedWorkCycles)) ? Number(parsed.completedWorkCycles) : 0,
    };
  } catch {
    return null;
  }
...
function getWeeklyRestCandidateForSelectedWeek(selectedSaturdayISO: string): WeeklyRestCandidate | null {
  const candidate = readWeeklyRestCandidate();
  if (!candidate) return null;
  // v5.2.0 standard model: the candidate is shown only on the next app week.
  if (selectedSaturdayISO <= candidate.closingSaturdayISO) return null;
  return candidate;
}

function getWeeklyRestTargets(candidate: WeeklyRestCandidate | null) {
  if (!candidate) return null;
  const reducedStart = candidate.finishAbs + 24 * 60;
  const fullStart = candidate.finishAbs + 45 * 60;
  const completedWorkCycles = candidate.completedWorkCycles || 0;
  const forceReduced = completedWorkCycles >= 6;
  return {
    reducedStart,
    fullStart,
    reducedOwedHours: 21,
    completedWorkCycles,
    forceReduced,
    primaryStart: forceReduced ? reducedStart : fullStart,
    primaryType: forceReduced ? "reduced" : "regular",
  };
}

function getWeeklyRestPrimaryStart(candidate: WeeklyRestCandidate | null, enabled: boolean): string {
  const targets = enabled ? getWeeklyRestTargets(candidate) : null;
  return targets ? absMinutesToLocalTime(targets.primaryStart) : "";
}

function getWeeklyRestCardData(candidate: WeeklyRestCandidate | null, current: DayRecord, enabled: boolean) {
  if (!enabled || !candidate || current.dayType !== "work") return null;
  const targets = getWeeklyRestTargets(candidate);
  if (!targets) return null;
  const helperEndAbs = getRestDisplayEndAbs(current) ?? getDayStartAbsMinutes(current);
  if (helperEndAbs > candidate.finishAbs + 72 * 60) return null;
  if (targets.forceReduced) {
    return {
      value: formatShortDayTime(targets.reducedStart),
      hint: t("from24hReducedWeeklyRest"),
      helper: `${t("option45Unavailable")} · ${t("sixWorkCyclesCompleted")} · ${t("compensationRequired")}`,
      tone: "reduced" as const,
    };
  }
  return {
    value: formatShortDayTime(targets.fullStart),
    hint: t("from45hWeeklyRest"),
    helper: `${t("option24h")}: ${formatShortDayTime(targets.reducedStart)} · ${t("compensationRequired")}`,
    tone: "regular" as const,
  };
}

function getWeeklyRestSuggestionHelp(candidate: WeeklyRestCandidate | null, current: DayRecord, enabled: boolean): string {
  // v5.2.0: weekly information lives in the dynamic Weekly Rest card, not under Start.
  return "";
}
...
function runDevTests() {
  ...
  const wrCandidate: WeeklyRestCandidate = { closingSaturdayISO: "2026-07-04", finishAbs: 1000, completedWorkCycles: 6 };
  console.assert(getWeeklyRestTargets(wrCandidate)?.primaryType === "reduced", "6 work cycles should prioritise 24h reduced weekly rest");
  console.assert(getWeeklyRestTargets({ ...wrCandidate, completedWorkCycles: 5 })?.primaryType === "regular", "under 6 work cycles should keep 45h weekly rest primary");
...
export default function App() {
  ...
  const weeklyRestWithinHelperWindow = Boolean(weeklyRestCandidate && getDayStartAbsMinutes(currentDay) <= weeklyRestCandidate.finishAbs + 72 * 60);
  const weeklyRestBaseActive = Boolean(weeklyRestCandidate && weeklyRestWithinHelperWindow && selectedSaturday > weeklyRestCandidate.closingSaturdayISO && currentDay.dayType === "work" && !hasCompletedWorkBeforeCurrentInThisWeek);
  const weeklyRestBasePrimaryStart = getWeeklyRestPrimaryStart(weeklyRestCandidate, weeklyRestBaseActive);
  ...
  const weeklyRestPrimaryStart = getWeeklyRestPrimaryStart(weeklyRestCandidate, weeklyRestCandidateActive);
  const weeklyRestCardData = getWeeklyRestCardData(weeklyRestCandidate, currentDay, weeklyRestCandidateActive);
  const weeklyRestSuggestionHelpRaw = getWeeklyRestSuggestionHelp(weeklyRestCandidate, currentDay, weeklyRestCandidateActive);
  ...
  const startFieldHint = !displayStartValue && dailyPrimarySuggestedStart
  ...
  function endWeek(type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) {
    ...
    const completedWorkCycles = finalDays.filter((d) => d.dayType === "work" && Boolean(d.finish)).length;
    if (weeklyAnchor) writeWeeklyRestCandidate({ closingSaturdayISO: closingSaturday, finishAbs: weeklyAnchor.finishAbs, completedWorkCycles });
    else writeWeeklyRestCandidate(null);
...
function WeeklyRestMiniCard({ data }: { data: { value: string; hint: string; helper: string; tone: "regular" | "reduced" } }) {
  const regular = data.tone === "regular";
  const colors = regular
    ? { border: "#86efac", bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", text: "#166534" }
    : { border: "#fde68a", bg: "linear-gradient(135deg,#ffffff 0%,#fef9c3 100%)", text: "#a16207" };
  return <div style={{ marginTop: 12, padding: 12, borderRadius: 14, border: `1px solid ${colors.border}`, background: colors.bg, color: colors.text, minHeight: 69, boxSizing: "border-box" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10 }}>
      <div style={{ fontSize: 12, fontWeight: 900 }}>{t("weeklyRest")}</div>
      <div style={{ fontSize: 11, fontWeight: 900, color: "#64748b" }}>{data.hint}</div>
    </div>
    <div style={{ fontSize: 22, fontWeight: 950, marginTop: 3, lineHeight: 1 }}>{data.value}</div>
    {data.helper && <div style={{ fontSize: 11, fontWeight: 900, marginTop: 4, lineHeight: 1.15 }}>{data.helper}</div>}
  </div>;
}
