...
function getSuggestedStartHelp(suggested: SuggestedStarts): string {
  ...
  } else if (suggested.h11 != null && suggested.h9 != null) {
...
type WeeklyRestCandidate = { closingSaturdayISO: string; finishAbs: number; completedWorkCycles?: number; createdAt?: string };
...
function readWeeklyRestCandidate(): WeeklyRestCandidate | null {
  ...
  try {
    ...
    const closingSaturdayISO = typeof parsed.closingSaturdayISO === "string" ? parsed.closingSaturdayISO : "";
    const finishAbs = Number(parsed.finishAbs);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(closingSaturdayISO) || !Number.isFinite(finishAbs)) return null;
    return {
      closingSaturdayISO,
      finishAbs,
...
function getWeeklyRestCandidateForSelectedWeek(selectedSaturdayISO: string): WeeklyRestCandidate | null {
  ...
  const expectedNextSaturday = toISODate(addDays(fromISODate(candidate.closingSaturdayISO), 7));
  return selectedSaturdayISO === expectedNextSaturday ? candidate : null;
}
...
type WeeklyRestPlan = { primaryText: string; primaryHelper: string; secondaryText: string; tone: "good" | "reduced" };

function getWeeklyRestPlan(candidate: WeeklyRestCandidate | null, current: DayRecord, hasCompletedWorkAfterCandidate: boolean): WeeklyRestPlan | null {
  if (!candidate || current.dayType !== "work" || current.start || current.finish || hasCompletedWorkAfterCandidate) return null;
  const targets = getWeeklyRestTargets(candidate);
  if (!targets) return null;
  const dayStartAbs = getDayStartAbsMinutes(current);
  if (dayStartAbs > candidate.finishAbs + 72 * 60) return null;
  const workCycles = Math.max(0, Math.floor(candidate.completedWorkCycles || 0));
  const forceReduced = workCycles >= 6;
  if (forceReduced) {
    return {
      primaryText: formatShortDayTime(targets.reducedStart),
      primaryHelper: t("from24hReducedWeeklyRest"),
      secondaryText: `${t("weekly45Unavailable")} · ${workCycles} ${t("workCyclesCompleted")} · +${targets.reducedOwedHours}h ${t("owedLater")}`,
      tone: "reduced",
    };
  }
  return {
    primaryText: formatShortDayTime(targets.fullStart),
    primaryHelper: t("from45hWeeklyRest"),
    secondaryText: `${t("weekly24Option")}: ${formatShortDayTime(targets.reducedStart)} (+${targets.reducedOwedHours}h ${t("owedLater")})`,
    tone: "good",
  };
}
...
export default function App() {
  ...
  const weeklyRestPlan = getWeeklyRestPlan(weeklyRestCandidate, currentDay, hasCompletedWorkBeforeCurrentInThisWeek);
  const weeklyRestCandidateActive = Boolean(weeklyRestPlan);
  const weeklyRestRequiredMinutes = weeklyRestPlan?.tone === "reduced" ? 24 * 60 : 45 * 60;
  ...
  const startFieldHint = !displayStartValue && dailyPrimarySuggestedStart
    ? (suggestedTimes.h11 != null ? t("from11hRest") : (!suggestedTimes.h9Blocked && suggestedTimes.h9 != null ? t("from9hRest") : ""))
    : "";
  ...
  function endWeek(type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) {
    ...
    const completedWorkCycles = finalDays.filter((day) => day.dayType === "work" && Boolean(day.finish)).length;
    writeWeeklyRestCandidate(weeklyAnchor ? { closingSaturdayISO: closingSaturday, finishAbs: weeklyAnchor.finishAbs, completedWorkCycles, createdAt: new Date().toISOString() } : null);
...
function WeeklyRestCard({ plan }: { plan: WeeklyRestPlan }) {
  const reduced = plan.tone === "reduced";
  const border = reduced ? "#fde68a" : "#86efac";
  const text = reduced ? "#a16207" : "#166534";
  const bg = reduced ? "linear-gradient(135deg,#ffffff 0%,#fef9c3 100%)" : "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)";
  return <div style={{ marginTop: 12, padding: 12, borderRadius: 14, border: `1px solid ${border}`, background: bg, color: text }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "start" }}>
      <div>
        <div style={{ fontSize: 12, color: text, fontWeight: 800 }}>{t("weeklyRestCard")}</div>
        <div style={{ fontSize: 18, fontWeight: 950, marginTop: 2 }}>{plan.primaryText}</div>
      </div>
      <div style={{ textAlign: "right", fontSize: 11, fontWeight: 900, lineHeight: 1.2, maxWidth: "48%" }}>{plan.primaryHelper}</div>
    </div>
    {plan.secondaryText && <div style={{ marginTop: 6, fontSize: 12, fontWeight: 850, color: reduced ? "#92400e" : "#166534", lineHeight: 1.22 }}>{plan.secondaryText}</div>}
  </div>;
}
