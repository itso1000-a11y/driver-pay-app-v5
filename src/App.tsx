...
function getWeeklyRestPlan(candidate: WeeklyRestCandidate | null, current: DayRecord, enabled: boolean) {
  ...
  const cycles = getWeeklyRestWorkCycleCount(candidate);
  const forceReduced = cycles >= 6;
  ...
  const helper = forceReduced
    ? `${t("weekly45Unavailable")} · ${t("sixWorkCyclesCompleted")} · ${t("compensationRequired")}`
    : `${t("weeklyRest24Option")}: ${formatShortDayTime(targets.reducedStart)} · ${t("compensationRequired")}`;
...
export default function App() {
  ...
  const weeklyRestCycles = getWeeklyRestWorkCycleCount(weeklyRestCandidate);
  const weeklyRestForceReduced = weeklyRestCycles >= 6;
  const weeklyRestBasePrimaryStart = getWeeklyRestPrimaryStart(weeklyRestCandidate ? { finishAbs: weeklyRestCandidate.finishAbs } : null, weeklyRestBaseActive, weeklyRestForceReduced);
  const weeklyRestTargets = getWeeklyRestTargets(weeklyRestCandidate);
  ...
  const weeklyRestSuggestionHelpRaw = "";
  ...
  const weeklyMinimumStartAbs = weeklyRestBaseActive && weeklyRestTargets ? weeklyRestTargets.reducedStart : null;
...
function WeeklyRestInlineCard({ plan }: { plan: { primaryValue: string; primaryHelp: string; helper: string } }) {
  return <div style={{ marginTop: 12, padding: "9px 12px", minHeight: 42, borderRadius: 14, border: "1px solid #eef2f7", background: "#f8fafc", display: "grid", gap: 3 }}>
    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "baseline" }}>
      ...
      <div style={{ fontSize: 11, color: "#64748b", fontWeight: 900 }}>{plan.primaryHelp}</div>
    </div>
    <div style={{ fontSize: 20, lineHeight: 1.05, color: "#0f172a", fontWeight: 950 }}>{plan.primaryValue}</div>
    {plan.helper && <div style={{ fontSize: 11, lineHeight: 1.15, color: plan.helper.includes("unavailable") || plan.helper.includes("невъзможна") ? "#b45309" : "#166534", fontWeight: 850 }}>{plan.helper}</div>}
  </div>;
}

function CompactWeekContext({ days, currentIndex }: { days: ComputedDay[]; currentIndex: number }) {
  const visibleDays = days.slice(0, Math.max(0, currentIndex) + 1);
  return <div style={sectionStyle}>
    <SectionHeading title={t("currentWeek")} />
      ...
      {visibleDays.map((day) => {
        const value = day.dayType === "work"
          ...
          : day.dayType === "holiday"
            ? t("holiday")
            : t("off");
        return <div key={day.id} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "6px 0", borderBottom: "1px solid #f1f5f9", fontSize: 14, fontWeight: 800 }}>
          <div style={{ color: "#475569" }}>{dayNameLabel(day.dayName)}</div>
          <div style={{ color: day.id === days[currentIndex]?.id ? "#0f172a" : "#334155", fontWeight: 900 }}>{value}</div>
        </div>;
      })}
    </div>
  </div>;
