...
function getWeeklyRestPlan(candidate: WeeklyRestCandidate | null, current: DayRecord, enabled: boolean) {
  ...
  const nowAbs = Math.floor(Date.now() / 60000);
  const reducedOptionIsActionable = historicalDay || targets.reducedStart >= nowAbs;
...
export default function App() {
  ...
  useEffect(() => { if (!actionMessage) return; const timer = window.setTimeout(() => setActionMessage(""), 2200); return () => window.clearTimeout(timer); }, [actionMessage]);
  ...
  const completedWeeklyRestInfo = weeklyRestBaseActive && displayStartValue
  ...
  useEffect(() => {
    ...
    if (completedWeeklyRestInfo?.reduced && completedWeeklyRestInfo.sourceKey && completedWeeklyRestInfo.sourceStartAbs != null && completedWeeklyRestInfo.compensationDeadlineISO) {
      if (!ledger.some((item) => item.sourceKey === completedWeeklyRestInfo.sourceKey)) {
        ledger = [...ledger, {
    ...
    if (enteredStartAbs != null && restBeforeMinutes != null) {
      const eligible = ledger
        .filter((item) => item.status === "outstanding" && enteredStartAbs > item.sourceStartAbs && currentDay.dateISO <= item.deadlineISO)
  ...
  }, [archiveMode, completedWeeklyRestInfo?.sourceKey, completedWeeklyRestInfo?.compensationMinutes, completedWeeklyRestInfo?.compensationDeadlineISO, completedWeeklyRestInfo?.sourceStartAbs, enteredStartAbs, restBeforeMinutes, currentDay.dateISO, weeklyRestCandidate?.closingSaturdayISO]);
  ...
  function endWeek(type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) {
    ...
    if (isWeekClosed(closingSaturday)) {
      if (!closedWeekHasChanges()) { setActionMessage(t("weekAlreadySaved")); return; }
...
function WeeklyRestInlineCard({ plan }: { plan: { primaryValue: string; primaryHelp: string; helper: string } }) {
  // The actionable weekly-rest Start is shown in the main Start field.
  // This card is explanation only, so it must not duplicate another competing proposal.
