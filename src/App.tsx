...
function TimeRow({ label, value, onChange, onBlur, placeholder = "Start", hint = "", invalid = false, errorHint = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; hint?: string; invalid?: boolean; errorHint?: string }) {
  ...
  return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 58, fontSize: displayFontSize, fontWeight: 900, textAlign: "center", letterSpacing: displayLetterSpacing, paddingLeft: 8, paddingRight: 8, paddingBottom: visibleHint ? 22 : 12, ...(invalid ? { borderColor: "#b91c1c", boxShadow: "0 0 0 3px rgba(185,28,28,.16)", color: "#b91c1c" } : {}) }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{visibleHint && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 900, color: invalid ? "#b91c1c" : "#64748b", pointerEvents: "none" }}>{visibleHint}</div>}</div></Field>;
}
