...
function buildWeeklyRestTimelineDays(currentDays: DayRecord[], archive: any[]): DayRecord[] {
  // Read-only cross-pay-period timeline. Current live data wins for the same date,
  // while archived weeks provide continuity across End Week boundaries.
...
function getWeeklyRestCycleSnapshot(currentDays: DayRecord[], archive: any[]): WeeklyRestCycleSnapshot {
  ...
  const timelineDays = buildWeeklyRestTimelineDays(currentDays, archive);
  const timeline = getWeeklyRestTimelineSnapshot(currentDays, archive);
...
function getWeeklyRestCycleSnapshotBeforeDate(currentDays: DayRecord[], archive: any[], cutoffDateISO: string): WeeklyRestCycleSnapshot {
  ...
  return getWeeklyRestCycleSnapshot(priorCurrentDays, priorArchive);
}
...
function getWeeklyRestPrimaryStart(anchor: { finishAbs: number } | null, enabled: boolean, forceReduced = false): string {
  ...
  if (!targets) return "";
  return absMinutesToLocalTime(forceReduced ? targets.reducedStart : targets.fullStart);
}
...
function getWeeklyRestPlan(anchor: { finishAbs: number } | null, current: DayRecord, enabled: boolean, forceReduced = false, cycles = 0) {
  ...
  const helperEndAbs = getRestDisplayEndAbs(current) ?? getDayStartAbsMinutes(current);
  if (helperEndAbs > anchor.finishAbs + 72 * 60) return null;
  const primaryAbs = forceReduced ? targets.reducedStart : targets.fullStart;
  const primaryHelp = forceReduced ? t("from24hReducedWeeklyRest") : t("from45hWeeklyRest");
...
function getWeeklyRestPalette(restMinutes: number | null, requiredMinutes: number) {
  ...
  if (restMinutes < 24 * 60) return { ...statusPalette("violation"), label: t("weeklyRestComplete") };
...
export default function App() {
  ...
  // Conservative due gate: when the factual cross-week timeline is known, a recent
  // qualifying weekly rest may suppress the legacy End Week candidate until six
  // completed work cycles have actually occurred after that rest. If the timeline is
  // unknown, preserve the legacy behaviour exactly. This gate only decides whether
  // the existing weekly-rest candidate may drive Start suggestions; it does not mark
  // days, mutate storage, create obligations, or change End Week behaviour.
  ...
  const legacyWeeklyRestBaseActive = Boolean(
    ...
    (weeklyRestDueByTimeline == null || weeklyRestDueByTimeline)
  );
  // Timeline-derived due state has priority because it follows the factual regulatory
  // cycle across pay-period boundaries. The legacy End Week candidate remains the
  // compatibility fallback when the cross-week timeline cannot establish a due anchor.
  const weeklyRestBaseActive = timelineWeeklyRestBaseActive || legacyWeeklyRestBaseActive;
  const weeklyRestAnchor = timelineWeeklyRestBaseActive
    ? timelineWeeklyRestAnchor
  ...
  const weeklyRestForceReduced = timelineWeeklyRestBaseActive ? false : weeklyRestCycles >= 6;
  ...
  const weeklyRestDisplayAnchor = timelineWeeklyRestAnchor
    ? timelineWeeklyRestAnchor
    : (weeklyRestCandidate && currentDayAfterWeeklyCandidate && (weeklyRestDueByTimeline == null || weeklyRestDueByTimeline)
  ...
  const weeklyRestDisplayForceReduced = timelineWeeklyRestAnchor ? false : weeklyRestDisplayCycles >= 6;
  ...
  const startFieldHint = !displayStartValue
    ? (weeklyRestCandidateActive && weeklyRestBasePrimaryStart
      ? (weeklyRestTargetIsBeforeSelectedDay && weeklyRestPrimaryAbs != null
        ? `${t("weeklyRestEnded")} ${formatShortDayTime(weeklyRestPrimaryAbs)}`
  ...
  const weeklyValidationTargets = timelineWeeklyRestPathEligible
    ...
    : weeklyRestTargets;
  const weeklyMinimumStartAbs = (timelineWeeklyRestPathEligible || legacyWeeklyRestBaseActive) && weeklyValidationTargets
    ? weeklyValidationTargets.reducedStart
    : null;
  const minimumLegalStartAbs = weeklyMinimumStartAbs ?? dailyMinimumStartAbs;
  const enteredStartAbs = displayStartValue ? getDayTimeAbsMinutes(currentDay, displayStartValue) : null;
  ...
  const startRestViolationText = seventhWorkCycleStartViolation
    ? t("weeklyRestStartRequired")
    : weeklyRestIncompleteStartViolation
      ? t("weeklyRestNotCompleted")
      : t("restNotCompleted");
  const weeklyRestPalette = weeklyRestBaseActive && displayStartValue ? getWeeklyRestPalette(restBeforeMinutes, weeklyRestRequiredMinutes) : null;
  ...
  const currentRestPalette = { ...statusPalette("unknown"), label: currentDay.dayType === "off" && currentDay.dateISO < toISODate(new Date()) ? t("restAtEndOfDay") : t("currentRest") };
  ...
  const completedWeeklyRestInfo = legacyWeeklyRestBaseActive && displayStartValue
  ...
  useEffect(() => {
    ...
    if (enteredStartAbs != null && restBeforeMinutes != null) {
      const restStartAbs = enteredStartAbs - restBeforeMinutes;
      const eligible = ledger
        .filter((item) => item.status === "outstanding" && restStartAbs >= item.sourceStartAbs && enteredStartAbs > item.sourceStartAbs && currentDay.dateISO <= item.deadlineISO)
        .sort((a, b) => a.deadlineISO.localeCompare(b.deadlineISO) || a.sourceStartAbs - b.sourceStartAbs);
      const obligation = eligible.find((item) => canCompleteWeeklyCompensation(restBeforeMinutes, item.originalMinutes));
      if (obligation) {
        ledger = ledger.map((item) => item.id === obligation.id ? { ...item, remainingMinutes: 0, status: "completed", completedByStartAbs: enteredStartAbs, completedRestMinutes: restBeforeMinutes } : item);
        changed = true;
      }
    }
  ...
  const restContextHelp = !displayStartValue ? "" : (weeklyRestBaseActive ? "" : getRestContextHelp(restBeforeMinutes));
  ...
  function seedWeeklyRestCandidateFromClosedPayPeriod(closingSaturday: string, finalDays: DayRecord[]) {
    // Compatibility bridge only: End Week still seeds the existing weekly-rest candidate.
    // Future weekly-rest timeline logic may supersede this candidate when a real mid-week
    // qualifying weekly rest is recognised, without changing pay-period close semantics.
...
function TimeRow({ label, value, onChange, onBlur, placeholder = "Start", hint = "", invalid = false, errorHint = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; hint?: string; invalid?: boolean; errorHint?: string }) {
  ...
  return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 58, fontSize: 24, fontWeight: 900, textAlign: "center", letterSpacing: 1, paddingLeft: 8, paddingRight: 8, paddingBottom: visibleHint ? 22 : 12, ...(invalid ? { borderColor: "#b91c1c", boxShadow: "0 0 0 3px rgba(185,28,28,.16)", color: "#b91c1c" } : {}) }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{visibleHint && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 900, color: invalid ? "#b91c1c" : "#64748b", pointerEvents: "none" }}>{visibleHint}</div>}</div></Field>;
}
...
function WeeklyRestInlineCard({ plan, showPrimary = false }: { plan: { primaryValue: string; primaryHelp: string; helper: string }; showPrimary?: boolean }) {
  // On Work days the actionable weekly-rest Start remains in the Start field.
  // On an Off day there is no Start field, so the same factual candidate context is
  // surfaced here without saving or accepting any proposed Start.
  return <div style={{ marginTop: 12, padding: "9px 12px", minHeight: 42, borderRadius: 14, border: "1px solid #eef2f7", background: "#f8fafc", display: "grid", gap: 3 }}>
    ...
    {showPrimary && <div style={{ fontSize: 11, lineHeight: 1.15, color: "#166534", fontWeight: 850 }}>{t("weeklyRestInProgress")}</div>}
    {showPrimary && plan.primaryValue && <div style={{ fontSize: 11, lineHeight: 1.15, color: "#166534", fontWeight: 850 }}>{plan.primaryHelp}: {plan.primaryValue}</div>}
    {plan.helper && <div style={{ display: "grid", gap: 2 }}>{plan.helper.split(" · ").map((line) => <div key={line} style={{ fontSize: 11, lineHeight: 1.15, color: line.includes("unavailable") || line.includes("невъзможна") ? "#b45309" : "#166534", fontWeight: 850 }}>{line}</div>)}</div>}
  </div>;
}
