...
const UI_TEXT: Record<Lang, Record<string, string>> = {
  en: {
    chooseLanguage: "Choose language", appTitle: "Driver Pay App V5.2.1", currentDay: "Current day", install: "Install", week: "Week", settings: "Settings", language: "Language",
  ...
  bg: {
    chooseLanguage: "Избери език", appTitle: "Driver Pay App V5.2.1", currentDay: "Текущ ден", install: "Инсталирай", week: "Седмица", settings: "Настройки", language: "Език",
...
function getWeeklyRestSuggestionHelp(anchor: { finishAbs: number } | null, current: DayRecord, enabled: boolean): string {
  // Kept for compatibility with older call sites; the visible v5.2.1 UI uses WeeklyRestInlineCard instead.
...
function WeeklyRestInlineCard({ plan }: { plan: { primaryValue: string; primaryHelp: string; helper: string } }) {
  return <div style={{ marginTop: 12, padding: "9px 12px", minHeight: 42, borderRadius: 14, border: "1px solid #dbeafe", background: "linear-gradient(135deg,#ffffff 0%,#eff6ff 100%)", display: "grid", gap: 3 }}>
    ...
    <div style={{ fontSize: 20, lineHeight: 1.05, color: "#1d4ed8", fontWeight: 950 }}>{plan.primaryValue}</div>
