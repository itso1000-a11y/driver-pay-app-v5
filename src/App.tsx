...
function getWeeklyRestCandidateForSelectedWeek(selectedSaturdayISO: string): WeeklyRestCandidate | null {
  ...
  // Backfill for users who closed the immediately previous pay week before this
  // version was installed. A stale older stored candidate must not hide the newer
  // factual previous-week anchor; prefer whichever applicable candidate is newer.
...
export default function App() {
  ...
  const weeklyRestBaseActive = timelineWeeklyRestBaseActive || endWeekWeeklyRestBaseActive;
  ...
  const weeklyMinimumStartAbs = timelineWeeklyRestPathEligible && weeklyValidationTargets
  ...
  const completedWeeklyRestInfo = legacyWeeklyRestBaseActive && hasFactualStart
    ? getLastCompletedWeeklyRestInfo(selectedSaturday, days)
    : null;
  const visibleReducedWeeklyCompensationMinutes =
