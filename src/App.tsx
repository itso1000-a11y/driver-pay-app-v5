...
function getWeeklyRestPalette(restMinutes: number | null, requiredMinutes: number) {
  ...
  return { ...statusPalette("reduced"), label: `${t("reducedWeeklyRest")} • ${t("compensationRequired")}` };
}
...
function getRestCardPalette(restMinutes: number | null, status: RestStatus, reducedCountBeforeCurrent: number) {
  ...
  if (restMinutes != null && restMinutes >= 24 * 60) return { ...statusPalette("reduced"), label: `${t("reducedWeeklyRest")} • ${t("compensationRequired")}` };
...
export default function App() {
  ...
  const startFieldHint = !displayStartValue
    ? (weeklyRestCandidateActive && weeklyRestBasePrimaryStart
      ...
      : (dailyPrimarySuggestedStart
        ...
        : ""))
    : "";
  const startFieldPlaceholder = weeklyRestCandidateActive && weeklyRestBasePrimaryStart ? weeklyRestBasePrimaryStart : (dailyPrimarySuggestedStart || t("start"));
  ...
  const restContextHelp = !displayStartValue ? "" : (weeklyRestBaseActive ? getWeeklyRestContextHelp(restBeforeMinutes, weeklyRestRequiredMinutes) : getRestContextHelp(restBeforeMinutes));
